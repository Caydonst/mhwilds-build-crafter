...
import { InboxArrowDownIcon, XMarkIcon } from "@heroicons/react/24/solid";
...
import ShareBuildContainer from "@/app/components/shareBuildContainer/shareBuildContainer";
...
export default function BuilderClient() {
  ...
  async function handleShare() {
    ...
    try {
      ...
      const sharedId = await shareBuild(build, savedBuildName);

      const url = `${window.location.origin}/build/${sharedId}`;

      setShareUrl(url);
      setShareBuildOpen(true);
    } catch (error) {
      console.error("Unable to share build:", error);
    } finally {
        ...
        <>
            ...
            <div className={styles.headerInfoContainer}>
              ...
              <div className={styles.headerBtnContainer}>
                ...
                <button
                  ...
                  onClick={handleShare}
                  disabled={shareLoading || isEqual(build, emptyBuild)}
                >
                  {shareLoading ? (
                    ...
                    <div className={styles.saveBtnInner}>
                      <ArrowUpOnSquareIcon className={styles.publishIcon} />
                      <span>Share</span>
                    </div>
                  )}
                </button>
                {!buildId ? (
                  <button
                    ...
                    onClick={handleSave}
                    disabled={
                      buildId ? loading || isEqual(build, savedBuild) : loading
                    }
                  >
                    {loading ? (
                      <div className={styles.saveSpinnerContainer}>
                ...
                <div className={styles.headerBtnContainer}>
                  ...
                  {!buildId ? (
                    <button
                      ...
                      disabled={
                        buildId
                          ? loading || isEqual(build, savedBuild)
                          : loading
                      }
                    >
                      {loading ? (
          ...
          <div className={styles.mobileFooter}>
            ...
            <div
              ...
              style={{ transform: `translateX(${sliderAmount}%)` }}
            ></div>
          </div>
          ...
          {gearSelectorOpen && (
            <GearSelector
              gearSelectorOpen={gearSelectorOpen}
              setGearSelectorOpen={setGearSelectorOpen}
              type={type}
              build={build}
              setBuild={setBuild}
            />
          ...
          {decoSelectorOpen && (
            <DecoSelector
              ...
              slotLevel={slotLevel}
              kind={decoKind}
              decoSelectorOpen={decoSelectorOpen}
              setDecoSelectorOpen={setDecoSelectorOpen}
              build={build}
              setBuild={setBuild}
              type={type}
            />
          ...
          <SaveBuildContainer
            saveBuildOpen={saveBuildOpen}
            setSaveBuildOpen={setSaveBuildOpen}
            buildName={buildName}
            setBuildName={setBuildName}
            saveBuild={saveBuild}
            saveBuildLoading={saveBuildLoading}
          />
          <BuildsFull
            buildsFullOpen={buildsFullOpen}
            setBuildsFullOpen={setBuildsFullOpen}
          />
          <ShareBuildContainer
            open={shareBuildOpen}
            setOpen={setShareBuildOpen}
            shareUrl={shareUrl}
          />
          {/*{!buildId && (
