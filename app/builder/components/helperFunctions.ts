...
export function findBonuses(
  ...
  if (build.weapon !== null) {
    ...
    const groupBonusId = skills.find(
      (skill) => skill.name === groupBonusName,
    )?.id;
    ...
    const foundGroupBonus = bonusesArray.find(
      (bonus) => bonus.id === groupBonusId,
    );
    if (foundGroupBonus) {
  ...
  for (const piece of pieces) {
    ...
    if (set) {
      ...
      if (set.groupBonus) {
        const groupBonuses = bonusesArray.find(
          (bonus) => bonus.id === set?.groupBonus?.skill.id,
        );
        if (groupBonuses) {
          groupBonuses.count++;
        } else {
      ...
      for (const skill of piece.skills) {
        if ("kind" in skill.skill && skill.skill.kind === "set") {
          const setBonus = bonusesArray.find(
            (bonus) => bonus.id === skill.skill.id,
          );
          if (setBonus) {
            setBonus.count++;
          } else {
...
export function updateStats(build: BuilderBuild) {
  ...
  if (buildWeapon) {
    ...
    buildStats.effectiveRaw = getEffectiveRaw(
        buildStats.raw,
        buildStats.affinity,
        getCriticalBoost(),
    );
    ...
    if (buildWeapon.specials.length > 0) {
      ...
      buildStats.elementDamage +=
        buildWeapon.specials[0].damage.display + getElementDamage();
    }
  }
...
function getEffectiveRaw(
    ...
    const normalizedAffinity = Math.max(-100, Math.min(100, affinity)) / 100;

    if (normalizedAffinity >= 0) {
        return raw * (
            1 + normalizedAffinity * (critMultiplier - 1)
        );
    }

    // Negative critical hits deal 75% raw damage.
    const negativeCritMultiplier = 0.75;

    return raw * (
        1 +
        Math.abs(normalizedAffinity) *
        (negativeCritMultiplier - 1)
    );
}
