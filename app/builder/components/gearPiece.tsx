...
export default function GearPiece({
  ...
  const sharpnessList: SharpnessKey[] = [
    "red",
    "orange",
    "yellow",
    "green",
  ...
  function isWeaponPiece(
    piece: WeaponArmorCharm | null | undefined,
  ): piece is Weapon {
    return !!piece && "specials" in piece;
  }

  function isArmorPiece(
    piece: WeaponArmorCharm | null | undefined,
  ): piece is Armor {
    return !!piece && "kind" in piece;
  }

  function isCharmRank(
    piece: WeaponArmorCharm | null | undefined,
  ): piece is CharmRank {
    return !!piece && "charm" in piece && !("kind" in piece);
  }
  function isCustomCharm(
    piece: WeaponArmorCharm | null | undefined,
  ): piece is CustomCharm {
  ...
  if (gearPiece) {
    ...
    const spriteX = isCharmRank(gearPiece)
      ? armorIndex.charm
  ...
  return (
    <div className={styles.buildPieceContainer}>
      {gearPiece !== null ? (
        isWeaponPiece(gearPiece) &&
        gearPiece.kind &&
        weapons.includes(gearPiece.kind) ? (
          <div className={styles.pieceContainerHeaderWrapper}>
            <div
              className={styles.pieceContainerHeader}
              onClick={readOnly ? undefined : () => openWeaponSelector(slotKey)}
            >
              <span
                className={`${styles.buildPieceIcon}`}
                style={{
                  backgroundPosition: `calc((-64px * ${weaponIndex[gearPiece.kind]}) * var(--build-icon-size)) calc((-64px * ${rarity}) * var(--build-icon-size))`,
                }}
              />
              <div className={styles.buildPieceInfo}>
                ...
                <div className={styles.weaponStatsContainer}>
                  <div className={styles.rawValContainer}>
                    <span
                      className={`${styles.statsIcon} ${styles.damageIcon}`}
                    ></span>
                    <p>{gearPiece.damage.raw}</p>
                  </div>
                  <div className={styles.affinityValContainer}>
                    <span
                      className={`${styles.statsIcon} ${styles.affinityIcon}`}
                    ></span>
                  ...
                  {gearPiece.specials.length !== 0 && (
                    <div className={styles.elementValContainer}>
                      {gearPiece.specials[0].hasOwnProperty("status") ? (
                        <>
                          <span
                            className={`${styles.statsIcon} ${styles[`${gearPiece.specials[0].status}`]}`}
                          ></span>
                        ...
                        <>
                          <span
                            className={`${styles.statsIcon} ${styles[`${gearPiece.specials[0].element}`]}`}
                          ></span>
                          <p>{gearPiece.specials[0].damage.display}</p>
                        </>
                      )}
                ...
                {gearPiece.sharpness && (
                  <div className={styles.weaponSharpness}>
                    {sharpnessList.map((sharpness) => (
                      <div
                        key={sharpness}
                        className={`${styles.sharpnessColor} ${styles[sharpness]}`}
                        style={{
                          width: `${(gearPiece.sharpness?.[sharpness] ?? 0 / 400) * 150}px`,
                        }}
                      ></div>
                    ))}
                  </div>
                ...
                <div className={styles.gearPieceSkillsContainer}>
                  {gearPiece.bonuses && (
                    <>
                      <p className={styles.setBonusSkillName}>
                        {gearPiece.bonuses.setBonus}
                      </p>
                      <p className={styles.groupSkillName}>
                        {gearPiece.bonuses.groupBonus}
                      </p>
                    </>
                  )}
                </div>
                <div className={styles.gearPieceSkillsContainer}>
                  ...
                  {gearPiece.reinforcements?.map((reinforcement, i) => (
                    <p
                      key={i}
                      className={`${reinforcement.lvl === "EX" ? styles.EX : styles.notEX}`}
                    >
                      {reinforcement.reinforcement} {reinforcement.lvl}
                    </p>
                  ))}
                </div>
              </div>
            ...
            {!readOnly && (
              <div
                ref={moreOptionsRef}
                className={styles.moreOptionsBtnContainer}
              >
                <button
                ...
                <div
                  className={
                    moreOptionsOpen
                      ? `${styles.moreOptionsContainer} ${styles.open}`
          ...
          <div className={styles.pieceContainerHeaderWrapper}>
            <div
              className={styles.pieceContainerHeader}
              onClick={readOnly ? undefined : () => openGearSelector(slotKey)}
            >
              <span
                className={styles.buildPieceIcon}
              ...
              <div className={styles.buildPieceInfo}>
                <p className={styles.pieceTitle}>{gearPiece?.name ?? ""}</p>
                <div className={styles.gearPieceSkillsContainer}>
                  {gearPiece.skills.map(
                    (skill, i) =>
                      "kind" in skill.skill &&
                      skill.skill.kind !== "set" &&
                      skill.skill.kind !== "group" && (
                        <p key={i}>
                          {skill.skill.name} {skill.level}
                        </p>
                      ),
                  ...
                  {findBonuses?.setBonuses.map((bonus, i) => (
                    <p key={i} className={styles.setBonusSkillName}>
                      {bonus}
                    </p>
                  ))}
                  {findBonuses?.groupBonuses.map((bonus, i) => (
                    <p key={i} className={styles.groupSkillName}>
                      {bonus}
                    </p>
                  ))}
                </div>
            ...
            {!readOnly && (
              <div
                ...
                className={
                  moreOptionsOpen
                    ? `${styles.moreOptionsBtnContainer} ${styles.open}`
                    : styles.moreOptionsBtnContainer
                }
              >
                <button
                  className={styles.moreOptionsBtn}
                    ...
                    <>
                      <div className={styles.optionBtnContainer}>
                        {gearPiece.transcendence === true ? (
                          <button
                            className={styles.transcendBtn}
                            onClick={() => {
                              updateTranscendence();
                              setMoreOptionsOpen((prev) => !prev);
                            }}
                          >
                            <ChevronDoubleDownIcon /> Untranscend
                          </button>
                        ) : (
                          <button
                            className={styles.transcendBtn}
      ...
      {(isWeaponPiece(gearPiece) || isArmorPiece(gearPiece)) &&
        gearPiece.slots.length > 0 && (
          <>
            <div className={styles.decoSlotsContainer}>
              {gearPiece.slots.map((s, i) => {
                ...
                const canFit =
                  placement?.slotLevel != null && placement.slotLevel <= s; // optional check
                ...
                return (
                  ...
                  >
                    {placement ? (
                      <span
                        className={`${styles.decoIcon} ${styles[`deco${getDecoSlot(s, placement)}`]}`}
                      />
                    ) : (
                      <span
                        className={`${styles.decoIcon} ${styles[`deco${s}`]}`}
                      />
                    )}
                    ...
                    {s > 0 && placement?.decoration && canFit && (
                      <div className={styles.slottedDeco}>
                        <p className={styles.inlaidDecoName}>
                          {placement.decoration.name}
                        </p>
                        {!readOnly && (
                          <button
                            className={styles.decoDeleteBtn}
                            onClick={(e) => {
                              e.stopPropagation();
        ...
        <>
          <div className={styles.decoSlotsContainer}>
            {gearPiece.slots.map((s, i) => {
              ...
              const placement = build?.decorations?.[slotKey]?.[i];
              const canFit =
                placement?.slotLevel != null && placement.slotLevel <= s.level; // optional check
              ...
              return (
                ...
                >
                  <span
                    className={`${styles.decoIcon} ${styles[`deco${s.level}`]}`}
                  />
